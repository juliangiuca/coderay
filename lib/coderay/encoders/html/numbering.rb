    ...
    class HTML
      ...
      module Numbering  # :nodoc:
        ...
        def self.number! output, mode = :table, options = {}
          ...
          case mode
          when :inline
            output = inline(start, bolding, output)

          when :table
            output = table(start, bolding, output)

          when :both
            output = both(start, bolding, output)
        ...
        private
        def self.line_count(output="")
          @line_count ||= output.count("\n")
        end

        def self.both(start, bolding, output)
          ...
          output.gsub!(/^.*$\n?/) do |line|
            ...
            line_number += 1
            "<div class=\"line-numbers\" id=\"line#{line_number - 1}\">#{line}</div>"
          end
          table(start, bolding, output)
        end

        def self.inline(start, bolding, output)
          max_width = (start + line_count).to_s.size
          line_number = start
          output.gsub!(/^.*$\n?/) do |line|
            line_number_text = bolding.call line_number
        ...
        def self.table(start, bolding, output)
